...
const Items = (props) => {
  ...
  //variables - paging
  const size = 30;
  const searchName = new URLSearchParams(window.location.search).get("searchName");
    ...
    <>
      <Container>
        ...
        <div className={"bg-light ms-3 me-3 mb-3"}>
          <div>
            <div><span className={"text-secondary"}>"{searchName}" 에 대한 상품</span></div>
          </div>
        </div>
        <Container>
          <div className={"d-flex justify-content-between mb-2"}>
            <div className={"d-flex justify-content-start align-items-center-2 m-0"}>
              <h4 className={"me-1 mb-0"}>"{searchName}" 에 대한 상품</h4>
              <div>( {totalCount} )</div>
            </div>
